import { Activity, Camera, Pause, Play, RotateCcw, Wifi, WifiOff } from 'lucide-react';
import { useEffect, useRef, useState, type ReactNode } from 'react';
import { beginCalibration, createCalibrationState, updateCalibration, type CalibrationSample } from '../motion/calibration';
...
export function MotionDebugPage() {
  const { videoRef, canvasRef, landmarks, fps, confidence, isRunning, error, start, stop } = usePoseTracker();
  const {
    connected,
    gameState,
    boostMessage,
    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-100">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-800 pb-4">
          <div>
            <h1 className="text-2xl font-bold tracking-normal">Zombie Run Motion Debug</h1>
            <p className="text-sm text-neutral-400">Client-side MediaPipe Pose plus Socket.IO game-state engine.</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button className="icon-button" onClick={isRunning ? stop : start} title="Toggle camera" type="button">
              <Camera size={18} />
            </button>
            <button className="icon-button" onClick={startCalibration} title="Calibrate" type="button">
              <Activity size={18} />
            </button>
            <button className="icon-button" onClick={current?.gameState === 'PAUSED' ? resume : pause} title="Pause" type="button">
              {current?.gameState === 'PAUSED' ? <Play size={18} /> : <Pause size={18} />}
            </button>
            <button className="icon-button" onClick={restart} title="Restart" type="button">
              <RotateCcw size={18} />
            </button>
          </div>
        </header>
        ...
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.55fr)]">
          <div className="relative overflow-hidden rounded border border-neutral-800 bg-black">
            ...
            {countdown ? (
              <div className="absolute inset-0 grid place-items-center bg-black/30 text-7xl font-black">{countdown}</div>
            ) : null}
          </div>
          ...
          <aside className="grid gap-3">
            <DebugCard title="Connection">
              ...
              />
              <Metric label="Game state" value={current?.gameState ?? 'loading'} />
            ...
            <DebugCard title="Calibration">
              ...
              <Metric label="Progress" value={`${Math.round(calibration.progress * 100)}%`} />
              <Metric label="Samples" value={String(calibration.sampleCount)} />
              <Metric label="Body scale" value={calibration.profile?.bodyScale.toFixed(3) ?? '-'} />
            </DebugCard>

            <DebugCard title="Motion">
              <Metric label="Running intensity" value={motion.runningIntensity.toFixed(2)} />
              <Metric label="Player speed" value={motion.playerSpeed.toFixed(2)} />
              <Metric label="Running" value={motion.isRunning ? 'yes' : 'no'} />
              <Metric label="Lane" value={motion.lane} />
              <Metric label="Jump" value={motion.jumpDetected ? 'detected' : 'idle'} />
              <Metric label="67 count" value={String(motion.sixtySevenCount)} />
            </DebugCard>

            <DebugCard title="Backend Game">
              <Metric label="Stamina" value={current ? `${current.stamina}/100` : '-'} />
              <Metric label="Zombie distance" value={current?.zombieDistance.toFixed(1) ?? '-'} />
              <Metric label="Survival time" value={current ? `${current.survivalTime.toFixed(1)}s` : '-'} />
              <Metric label="Combo" value={String(current?.comboCount ?? 0)} />
              <Metric label="Obstacle" value={current?.currentObstacle?.type ?? 'none'} />
              <Metric label="Score" value={String(Math.round(current?.score ?? 0))} />
            </DebugCard>

            {boostMessage ? <div className="rounded border border-lime-400 bg-lime-400 px-3 py-2 text-center font-bold text-neutral-950">{boostMessage}</div> : null}
          </aside>
        </section>
      </div>
    </main>
...
function DebugCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="rounded border border-neutral-800 bg-neutral-900 p-3">
      <h2 className="mb-2 text-sm font-semibold uppercase text-neutral-400">{title}</h2>
